...
class GridGenerator:
    ...
    def __init__(
        self, config_object, projection_definition, grid_definition
    ):  # , logger = None):
        """
    ...
    def generate_grid_xy_ease2(
        self, return_resolution: bool = False
    ) -> (
    ...
    def generate_grid_xy_mercator(
        ...
        min_lat, max_lat = (
            GRIDS[self.grid_definition]["lat_min"],
            GRIDS[self.grid_definition]["lat_max"],
        )  # Latitude range in degrees (Mercator excludes poles)

        # Define resolution in meters
        resolution_m = GRIDS[self.grid_definition][
            "res"
        ]  # Set the desired grid resolution in meters (e.g., 10 km)
        ...
        num_x = (
            int((max_x - min_x) / resolution_m) + 1
        )  # Number of points in the x-direction
        num_y = (
            int((max_y - min_y) / resolution_m) + 1
        ...
        y = np.linspace(
            max_y, min_y, num_y
        )  # Evenly spaced y-coordinates, top-to-bottom
    ...
    def generate_grid_xy(
        ...
        if "EASE2" in self.grid_definition:
            ...
            raise NotImplementedError(
                f"Grid {self.grid_definition} is not implemented."
            )
    ...
    def generate_grid_lonlat(self):
        ...
        tracked_func = RGBLogging.rgb_decorate_and_execute(
            decorate=self.decorate, decorator=RGBLogging.track_perf, logger=self.logger
    ...
    def lonlat_to_xy_laea(
        self, lon: np.ndarray | float, lat: np.ndarray | float, pole: str = "N"
    ) -> tuple[np.ndarray | float, np.ndarray | float]:
        """
        ...
        lon_0 = float(
            next((param.split("=")[1] for param in params if "lon_0" in param), None)
        )
        dlon = lon - lon_0
        phi = np.deg2rad(lat)
        lam = np.deg2rad(dlon)
    ...
    def lonlat_to_xy_cea(
        ...
        lat_ts_value = float(
            next((param.split("=")[1] for param in params if "lat_ts" in param), None)
        )
    ...
    def xy_to_lonlat_laea(
        self, x: np.ndarray | float, y: np.ndarray | float, pole: str
    ) -> tuple[np.ndarray | float, np.ndarray | float]:
        """
    ...
    def xy_to_lonlat_stereo(
        self, x: np.ndarray | float, y: np.ndarray | float, pole: str = "N"
    ) -> tuple[np.ndarray | float, np.ndarray | float]:
        """
        Polar Stereographic projection using pyproj library:
        https://proj.org/en/9.5/operations/projections/stere.html
    ...
    def get_grid_area(self):
        if self.grid_area is None:
            ...
            if "EASE2" in self.grid_definition:
                ...
                target_dir = pkg_resources.files(
                    "cimr_rgb.dpr.Grids.NSIDC_PS"
                )  # Adjust the package path if necessary

                # Iterate through files in the target directory
                for file in target_dir.iterdir():
                    if (
                        f"{self.projection_definition}{str(int(resolution / 1000))}"
                        in file.name
                    ):
