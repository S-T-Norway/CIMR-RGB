...
def interp_gain_in_chunks(gain: np.ndarray, n_rows: int, n_cols: int, 
                          X, Y, x_down, y_down,  
                          num_chunks: int = 4, overlap_margin: float = 0.1) -> np.ndarray: 
...
def interp_beamdata_into_uv(cimr: dict(), chunk_data: bool = True) -> dict(): 
    """
    ...
    #        cimr["temp"][key] = interp_gain(x_down = u, y_down = v, temp = cimr["temp"][key], 
    #                                        X = cimr["Grid"]["u_grid"],  
    #                                        Y = cimr["Grid"]["v_grid"], 
    #                                        ).T  
    #                                        #X = U, 
