...
def get_max_index(G: np.ndarray) -> tuple[int, int]:
    """
...
def generate_uv_grid(
    ...
    u_bounds = (xcen + xs, xcen + xs + dx * (nx - 1))
    v_bounds = (ycen + ys, ycen + ys + dy * (ny - 1))
    ...
    # u_grid, v_grid = np.mgrid[u0:u1:dx, v0:v1:dy]

    # Computing the bounds of the gird and if it is not whitin those then cropping the values
    u_min, u_max = np.min(u_grid), np.max(u_grid)
    v_min, v_max = np.min(v_grid), np.max(v_grid)
    if not (u_bounds[0] <= u_min and u_max <= u_bounds[1]):
        print(
            f"Error: u_grid values out of bounds. Expected {u_bounds}, but got ({u_min}, {u_max})"
        )
        print("Removing out of bounds values")
        u_grid = np.clip(u_grid, u_bounds[0], u_bounds[1])
        u_min, u_max = np.min(u_grid), np.max(u_grid)
        print(f"It now has: ({u_min}, {u_max})")
        u_grid = u_grid[:nx, :ny]

    if not (v_bounds[0] <= v_min and v_max <= v_bounds[1]):
        print(
            f"Error: v_grid values out of bounds. Expected {v_bounds}, but got ({v_min}, {v_max})"
        )
        print("Removing out of bounds values")
        v_grid = np.clip(v_grid, v_bounds[0], v_bounds[1])
        v_min, v_max = np.min(v_grid), np.max(v_grid)
        print(f"It now has: ({v_min}, {v_max})")
        v_grid = v_grid[:nx, :ny]

    # if u_grid.shape != (nx, ny) or v_grid.shape != (nx, ny):
    #     print(f"Error: Expected shape {(nx, ny)}, but got {u_grid.shape}")
    # exit()
...
def construct_complete_gains(cimr: dict) -> dict:
    ...
    # print(cimr["Gain"]["G1h"].shape)
    # print(cimr["temp"]["Ghh"].shape)
...
def interp_gain(
    ...
    print(f"2. [interp_gain] Ghh: {np.shape(gain)}")
    print(f"2. [interp_gain] X: {np.shape(X)}")
    grid_points = np.vstack([X.ravel(), Y.ravel()]).T

    gain = sp.interpolate.griddata(
        grid_points, gain, (x_down, y_down), method=interp_method
    )

    return gain


def interp_gain_in_chunks(
    gain: np.ndarray,
    n_rows: int,
    n_cols: int,
    x_lin: np.ndarray,
    ...
    for i in range(0, num_chunks + 1, 1):
        ...
        for j in range(0, num_chunks + 1, 1):
            ...
            if j == 0:
                ...
                x_max_list[j] = (
                    x_origin_chunk.max()
                    - overlap_margin * chunk_size_row * abs_length / n_rows
                )
...
def interp_beamdata_into_uv(
    ...
    print(f"1. [interp_beamdata_into_uv] Ghh: {np.shape(cimr['temp']['Ghh'])}")
    # New grid
    grid_points_theta = (
        int(grid_max_theta / grid_res_theta) + 1
    )  # +1 added to include the endpoint
    grid_points_phi = int(360.0 / grid_res_phi) + 1
    ...
    print(f"2. [interp_beamdata_into_uv] Ghh: {np.shape(cimr['temp']['Ghh'])}")
    print(f"2. [interp_beamdata_into_uv] x: {np.shape(x)}")
    ...
    for key in list(cimr["temp"].keys()):
        ...
        if chunk_data:
            cimr["temp"][key] = interp_gain_in_chunks(
                gain=cimr["temp"][key],
                n_cols=cimr["Grid"]["nx"],
                n_rows=cimr["Grid"]["ny"],
                x_lin=cimr["Grid"]["u"],
        ...
        cimr["temp"][key] = cimr["temp"][key].reshape(
            grid_points_phi, grid_points_theta
        )

        if key == "Ghh":
            cimr["Gain"]["G1h"], cimr["Gain"]["G2h"] = (
                np.real(cimr["temp"][key]),
                np.imag(cimr["temp"][key]),
            )
        elif key == "Ghv":
            cimr["Gain"]["G3h"], cimr["Gain"]["G4h"] = (
                np.real(cimr["temp"][key]),
                np.imag(cimr["temp"][key]),
            )
        elif key == "Gvv":
            cimr["Gain"]["G1v"], cimr["Gain"]["G2v"] = (
                np.real(cimr["temp"][key]),
                np.imag(cimr["temp"][key]),
            )
        elif key == "Gvh":
            cimr["Gain"]["G3v"], cimr["Gain"]["G4v"] = (
                np.real(cimr["temp"][key]),
                np.imag(cimr["temp"][key]),
            )
