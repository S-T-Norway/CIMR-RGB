...
### Added
- `uv` and `uv2nix` configurations

### Changed 
- `pyproject.toml` to accomodate `uv`
- `nix` configuration by removing `shell.nix` and relying more on `uv.lock` file generated from the `pyproject.toml` 
- `cimr-grasp` to accept both polarizations
