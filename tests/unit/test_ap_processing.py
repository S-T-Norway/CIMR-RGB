...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            -73.778691, 40.639928, -0.461388, 51.477500, 5539429, id="NYC_to_London"
        ),
        pytest.param(
            0, 0, 180, 0, np.pi * ((6378137.0 + 6356752.0) / 2.0), id="antipodal_points"
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "G", "EASE2_G3km", 0, 0, 3000, (lon1, lat1), id="regular_global_grid"
        ),
        pytest.param(
            "G",
            "EASE2_G3km",
        ...
        pytest.param(
            ...
            0,
            (lon5, lat5),
            id="polar_grid_across_idl",
        ),
    ],
)
def test_make_integration_grid(
    int_projection_definition,
    int_grid_definition,
    longitude,
    latitude,
    ap_radii,
    ...
    lon, lat = ap.make_integration_grid(
        int_projection_definition, int_grid_definition, longitude, latitude, ap_radii
    )
    lonexp, latexp = expected

    assert lon.shape == lonexp.shape, (
        f"Expected size of longitude grid {lonexp.shape}, got {lon.shape}"
    )
    assert lat.shape == latexp.shape, (
        f"Expected size of latitude grid {latexp.shape}, got {lat.shape}"
    )
...
@pytest.mark.parametrize(
    ("alpha", "R", "H", "expected"),
    [
        pytest.param(0, 1.0, 1.0, (0.0, 1.0), id="nadir"),
        pytest.param(
            30.0,
            3.0,
            1.0,
            (np.sqrt(3) - np.sqrt(5) / 2.0, 1 + np.sqrt(15) / 2.0),
            id="30deg",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            0.0,
            [1.0, 1.0],
            1.1 * 3492.1105641045438,
            id="gaussian_2deg_from_nadir",
        ),
        pytest.param(
            0,
            100000.0,
            ...
            None,
            1.1 * 3492.1105641045438,
            id="instrument_2deg_from_nadir",
        ),
    ],
)
def test_estimate_max_ap_radius(
    mocker,
    tilt_angle,
    max_altitude,
    max_theta_antenna_patterns,
    ...
    config.antenna_patterns_path = repo_root.joinpath(
        "tests", "unit", "mock_antenna_patterns"
    ).resolve()
    config.max_altitude = max_altitude
    config.max_theta_antenna_patterns = max_theta_antenna_patterns

    antenna_pattern = ap.AntennaPattern(
        config, "L", antenna_method, "scalar", antenna_threshold, gaussian_params
    )

    assert antenna_pattern.max_ap_radius[0] == expected, (
        f"Expected {expected}, got {antenna_pattern.max_ap_radius[0]}"
    )
...
lon1, lat1 = ap.make_integration_grid("G", "EASE2_G3km", 0.0, 0.0, 10000.0)
pos1 = [6378137.0 + 500000.0, 0.0, 0]
vel1 = [1.0, 0.0, 0.0]
...
def test_antenna_pattern_to_earth(
    mocker,
    instrument,
    tilt_angle,
    int_dom_lons,
    ...
    repo_root = grasp_io.find_repo_root()
    if instrument == "SMAP":
        # config.antenna_patterns_path = (
        #     "./mock_antenna_patterns/L/CIMR-PAP-FR-L0-TPv1.0.0.h5"
        # )
        config.antenna_patterns_path = repo_root.joinpath(
            "tests", "unit", "mock_antenna_patterns", "L", "CIMR-PAP-FR-L0-TPv1.0.0.h5"
        ).resolve()
    else:
        # config.antenna_patterns_path = "./mock_antenna_patterns"
        config.antenna_patterns_path = repo_root.joinpath(
    ...
    config.max_altitude = 100000.0
    config.max_theta_antenna_patterns = 10.0
    config.scan_angle_feed_offset = {"L": {0: 0.0}}
    ...
    ginterp = antenna_pattern.antenna_pattern_to_earth(
        int_dom_lons,
        int_dom_lats,
        pos[0],
        pos[1],
    ...
    assert np.isclose(ginterp, expected, rtol=1e-2).all(), (
        "Projected gain is different from expected"
    )
