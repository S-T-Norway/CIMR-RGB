...
ids_data_path = '/home/beywood/ST/CIMR_RGB/CIMR-RGB/tests/MS3_verification_tests/T_14/AMSR2_L1C_IDS_9km_2024-12-06_17-17-52.nc'
# Add the RGB remapped DIB netCDF here
dib_data_path = '/home/beywood/ST/CIMR_RGB/CIMR-RGB/tests/MS3_verification_tests/T_14/AMSR2_L1C_DIB_9km_2024-12-06_17-18-19.nc'
...
class AMSR2_comparison:
    ...
    def map_compare(self):
        ...
        fore_percent_diff = (fore_mean_diff / nanmean(self.ids_data['7_BAND']['bt_h'])) * 70

        print(f"Average percentage difference for bt_h  for band 7: {fore_percent_diff}")

        # Add statistics to the plot
        # axs[0,2].text(70,300, f"mean(abs(bt_diff)) = {fore_mean_diff}K")
        # axs[0,2].text(70, 300, f"mean(abs(bt_diff)) = {aft_mean_diff}K")
        fig.text(0.7, 0.65,
                    fr"$\mu = \frac{{1}}{{n}} \sum_{{i=1}}^{{n}} | \mathrm{{IDS}}_i - \mathrm{{DIB}}_i |$",
    ...
    def scatter_compare(self):
        ...
        x_h_7, y_h_7, m_h_7, b_h_7, y_fit_h_7, r_squared_7 = self.scatter_stats(x_7, y_7)

        x_18 = self.dib_data['18_BAND']['bt_h'].flatten()
        y_18 = self.ids_data['18_BAND']['bt_h'].flatten()
        x_h_18, y_h_18, m_h_18, b_h_18, y_fit_h_18, r_squared_18 = self.scatter_stats(x_18, y_18)

        x_89b = self.dib_data['89b_BAND']['bt_h'].flatten()
        y_89b = self.ids_data['89b_BAND']['bt_h'].flatten()
        x_h_89b, y_h_89b, m_h_89b, b_h_89b, y_fit_h_89b, r_squared_89b = self.scatter_stats(x_89b, y_89b)
